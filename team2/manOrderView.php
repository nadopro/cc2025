<?php
// manOrderView.php
// 사용: index.php?cmd=manOrderView&order_idx=2
...
// (선택) 관리자 체크 - 필요 없으면 이 블럭은 주석 처리
if (empty($_SESSION['sino_level']) || (int)$_SESSION['sino_level'] !== 9) {
    echo '<div class="container py-4"><div class="alert alert-danger">관리자만 접근 가능합니다.</div></div>';
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['new_status'])) {
    ...
    if (!isset($statusNames[$newStatus])) {
        $msg = "잘못된 상태 코드입니다.";
    } else {
        $sqlUpdate = "UPDATE order_table SET status = ? WHERE idx = ?";
        $stmtUpdate = $conn->prepare($sqlUpdate);
        $stmtUpdate->bind_param("ii", $newStatus, $orderIdx);

        if ($stmtUpdate->execute()) {
            // 새로고침 시 중복 제출 방지용 간단한 리다이렉트
            echo "<script>alert('주문 상태가 변경되었습니다.');location.href='index.php?cmd=manOrderView&order_idx={$orderIdx}';</script>";
            exit;
        } else {
            $msg = "상태 변경 실패: " . htmlspecialchars($stmtUpdate->error);
        }
    }

    if (!empty($msg)) {
        echo "<div class='container py-2'><div class='alert alert-danger'>{$msg}</div></div>";
    }
}
...
$stmtOrder->bind_param("i", $orderIdx);
$stmtOrder->execute();
$resOrder = $stmtOrder->get_result();
$order = $resOrder->fetch_assoc();

if (!$order) {
    echo '<div class="container py-4"><div class="alert alert-warning">해당 주문을 찾을 수 없습니다.</div></div>';
    exit;
}
...
<div class="container py-4">
    ...
    <!-- 상태 변경 폼 -->
    <form method="post" action="index.php?cmd=manOrderView&order_idx=<?= $orderIdx ?>" class="mb-4">
        <div class="row g-2 align-items-center">
            <div class="col-auto">
                <label for="new_status" class="col-form-label fw-semibold">상태 변경</label>
            </div>
            <div class="col-auto">
    ...
    <div class="text-end">
        <a href="index.php?cmd=manOrder&status=<?= $currentStatus ?>" class="btn btn-outline-secondary">
            목록으로
        </a>
    </div>
</div>
